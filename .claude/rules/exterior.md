---
paths:
  - "Assets/Scripts/ResidenceViz/ExteriorBridge.cs"
  - "Assets/Scripts/ResidenceViz/Tools/OutdoorTool.cs"
...
`ResidenceDoc.exteriorEnabled` is `false` and `VariantDef.exterior` is `null` until the **Outdoors**
...
bare terrain; `ResidenceRenderer` keeps exactly one of `GroundPad` / `Terrain` active. `WorldRenderer`'s
tile-building fields are deliberately unassigned in `ResidenceViz.unity` (`buildingInstances` is always
