# CXRBrownfield — legacy site-visioning tool

> **This is the legacy half of the repository.** CXRHomeViz is the product; see
> [`../CLAUDE.md`](../CLAUDE.md). Nothing in the Brownfield stack was deleted — `EditController`,
...
the default PC build**). The Brownfield and VR items live under a **`Legacy — Brownfield`** submenu,
because this repo's product is CXRHomeViz; they were demoted, not deleted, so the Brownfield stack that
is still in this repo remains buildable:
- **Build → Legacy — Brownfield → Desktop (PC, Windows)** (`Ctrl+Shift+D`) — the PC app as before,
  ships only `BasicModel`, no VR.
- **Build → Legacy — Brownfield → VR — Quest (Android)** (`Ctrl+Shift+Q`) — ships only `VRViewer`;
  OpenXR is already enabled for the Android target.
- **Build → Legacy — Brownfield → VR — PCVR (Windows)** — ships only `VRViewer`; **requires** enabling
