...
[TestFixture]
public class ResidenceMetricsTests
{
    ...
    public void ClearWidth_MeasuredValueBeatsTheEstimate()
    {
        ...
        Assert.AreEqual(0.85f, ResidenceMetrics.ClearWidth(o), 1e-4f);
        Assert.IsTrue(ResidenceMetrics.IsClearWidthMeasured(o));
    }
    ...
    public void ClearWidth_DoorLosesTheLeafAndStop()
    {
        ...
        Assert.AreEqual(0.9144f - 0.060f, ResidenceMetrics.ClearWidth(o), 1e-4f);
        Assert.IsFalse(ResidenceMetrics.IsClearWidthMeasured(o));
    }
    ...
    public void ClearWidth_CasedOpeningHasNoLeafToLose()
    {
        ...
        Assert.AreEqual(0.9144f, ResidenceMetrics.ClearWidth(o), 1e-4f);
    }
    ...
    public void ClearWidth_WindowHasNoLeafToLose()
    {
        ...
        Assert.AreEqual(1.2f, ResidenceMetrics.ClearWidth(o), 1e-4f);
    }
    ...
    public void Threshold_IsDetected()
    {
        ...
        Assert.IsFalse(ResidenceMetrics.HasThreshold(flush));

        var raised = Door(0.9f);
        raised.thresholdHeight = 0.019f;   // a 3/4" strip: a real trip hazard
        Assert.IsTrue(ResidenceMetrics.HasThreshold(raised));
    }
    ...
    public void RoomAreaAndPerimeter()
    {
        ...
        Assert.AreEqual(12f, ResidenceMetrics.RoomArea(room), 1e-3f);
        Assert.AreEqual(14f, ResidenceMetrics.RoomPerimeter(room), 1e-3f);
    }
    ...
    public void RoomCentroid_OfARectangleIsItsMiddle()
    {
        ...
        var c = ResidenceMetrics.RoomCentroid(room);
    ...
    public void RoomAt_FindsTheContainingRoomAndIgnoresDegenerateOnes()
    {
        ...
        Assert.AreEqual("bath", ResidenceMetrics.RoomAt(new Vector2(2f, 1.5f), level)?.id);
        Assert.IsNull(ResidenceMetrics.RoomAt(new Vector2(50f, 50f), level));
    }
    ...
    {
        var circle = ResidenceMetrics.LargestInscribedCircle(RectPoly(0, 0, 4, 4));
    ...
    public void LargestInscribedCircle_InACorridorIsLimitedByTheNarrowDimension()
    {
        ...
        var circle = ResidenceMetrics.LargestInscribedCircle(RectPoly(0, 0, 6, 2));
    ...
    public void LargestInscribedCircle_ConcaveRoomIsSmallerThanItsBoundingBox()
    {
        ...
        var circle = ResidenceMetrics.LargestInscribedCircle(poly);
    ...
    {
        Assert.IsFalse(ResidenceMetrics.LargestInscribedCircle((IReadOnlyList<Vector2>)null).valid);
        Assert.IsFalse(ResidenceMetrics.LargestInscribedCircle(new List<Vector2>
        {
    ...
    public void PointOnWall_InterpolatesAndClamps()
    {
        ...
        Assert.AreEqual(1f, ResidenceMetrics.PointOnWall(w, 1f).x, 1e-4f);
        Assert.AreEqual(4f, ResidenceMetrics.PointOnWall(w, 99f).x, 1e-4f);
        Assert.AreEqual(0f, ResidenceMetrics.PointOnWall(w, -5f).x, 1e-4f);
    }
    ...
    public void WallMidpoint()
    {
        ...
        var m = ResidenceMetrics.WallMidpoint(w);
    ...
    {
        Assert.AreEqual(1f, ResidenceMetrics.PointSegmentDistance(
        ...
        Assert.AreEqual(5f, ResidenceMetrics.PointSegmentDistance(
            new Vector2(10, 0), new Vector2(0, 0), new Vector2(5, 0)), 1e-4f);

        Assert.AreEqual(3f, ResidenceMetrics.PointSegmentDistance(
