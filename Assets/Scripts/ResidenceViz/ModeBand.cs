...
//
// ResidenceViz is two editors wearing one face. Editing the base environment. Making the model match the
// residence that actually exists, and proposing a change to it are different acts with different
...
// ResidenceToolBase.RefuseIfLocked put a third in every tool rail, and nothing said which was which.
//
...
// ResidenceDoc without notifying anyone, so a held VariantDef would be a stale object describing a
// variant that no longer exists.
//
// There is no subtitle. Per this app's rule every caption is a tooltip, and a band that printed
// "the residence as it is today" under its own title would be exactly the prose the rest of the UI had
...
public class ModeBand
{
    ...
    /// </summary>
    public static float HeightFor(ResidenceDoc doc, bool menuOpen)
    {
    ...
    /// </summary>
    public void Draw(Rect area, ResidenceDoc doc, VariantDef variant, int changeCount,
    ...
    private void DrawTitle(ResidenceDoc doc, VariantDef variant, Mode mode, int changeCount,
                           string baselineName, float available)
    {
        ...
        // be called ("Existing" on every residence this app has ever written). Its name still appears in
    ...
    private void DrawMenu(ResidenceDoc doc, VariantDef current)
    {
        foreach (var v in doc.variants)
        {
            ...
            UITheme.Tip(v.isBaseline
                ? "The residence as it stands. Switching is instant."
    ...
    private static string TitleTip(Mode mode, string baselineName, bool switchable)
    {
        ...
        {
            Mode.EditingBase =>
                "Edits here change the residence as it stands." + tail,
            Mode.Base =>
                "The residence as it stands. Read-only." + tail,
            _ =>
