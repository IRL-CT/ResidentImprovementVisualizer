...
//
//   * NOT IN A TOOL. ResidenceEditController gates IResidenceTool.DrawOverlay on !PointerOverUI, so an overlay
...
//
// Everything is derived from the schema each frame rather than cached. Undo restores the whole ResidenceDoc
...
public class SelectionOverlay
{
    ...
    public void Draw(Camera cam, LevelDef level, VariantDef variant,
                     ResidenceElementMarker.Kind kind, string id, ResidenceRenderer renderer)
    {
        if (cam == null || level == null || string.IsNullOrEmpty(id)) return;

        switch (kind)
        {
            case ResidenceElementMarker.Kind.Wall:      DrawWall(cam, level, id); break;
            case ResidenceElementMarker.Kind.Opening:   DrawOpening(cam, level, id); break;
            case ResidenceElementMarker.Kind.Room:
            case ResidenceElementMarker.Kind.Floor:
            case ResidenceElementMarker.Kind.Ceiling:   DrawRoom(cam, level, id); break;
            case ResidenceElementMarker.Kind.Furniture: DrawFurniture(cam, level, id); break;
            case ResidenceElementMarker.Kind.WallMount: DrawMount(cam, level, id); break;
            case ResidenceElementMarker.Kind.Sensor:    DrawSensor(cam, level, variant, id); break;
            case ResidenceElementMarker.Kind.Occupant:  DrawOccupant(cam, level, variant, id, renderer); break;
        }
    }
    ...
    private void DrawRoom(Camera cam, LevelDef level, string id)
    {
        ...
        // the question you are actually asking. ResidenceMetrics still computes it; OccupancyModel stands
        // people with it and the walkthrough spawns with it.
        if (OverlayDraw.ToScreen(cam, ResidenceMetrics.RoomCentroid(room), y, out Vector2 mid))
    ...
    // The TRUE rotated rectangle, not the axis-aligned bound. ResidenceMetrics.FootprintOf snaps to quarter
    ...
    {
        var s = ResidenceRenderer.FindSensor(id, level);
        if (s == null) return;
    ...
    private void DrawMount(Camera cam, LevelDef level, string id)
    {
        ...
        Vector2 at = ResidenceMetrics.PointOnWall(wall, m.offset);
