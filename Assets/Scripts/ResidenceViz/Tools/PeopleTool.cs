...
// timeline. It lives on ResidenceRenderer, and the TimelineBar is the one place it is read and scrubbed.
public class PeopleTool : ResidenceToolBase
{
    ...
    public override void HandleInput()
    {
        ...
        if (Ctx.PickElement(out ResidenceElementMarker marker, out _) &&
            marker.kind == ResidenceElementMarker.Kind.Occupant)
        {
    ...
    private void DrawRoster(List<OccupantDef> people)
    {
        ...
        foreach (var p in people)
        {
            ...
            Ctx.Controller.Select(ResidenceElementMarker.Kind.Occupant, p.id, reveal: false);
            _editingActivity = null;
    ...
    private void DrawActivityEditor(OccupantDef person, ActivityDef a)
    {
        ...
        UITheme.Tip("Away from residence. Their marker hides for this block");
    ...
    private void AddPerson()
    {
        ...
        Ctx.Controller.Select(ResidenceElementMarker.Kind.Occupant, person.id, reveal: false);
        TouchRoster();
    }
    ...
    {
        if (Ctx?.Controller == null || Ctx.Controller.SelectedKind != ResidenceElementMarker.Kind.Occupant)
            return null;
