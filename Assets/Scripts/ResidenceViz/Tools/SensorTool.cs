...
// different tab, which is a hard stop in a tool used to lay a residence out before deciding who moves in.
//
...
// always did. Nothing downstream needed a line for either, and no figure about what the residence can see
// moves. See SensorFit.Personal for why both of those are true by construction.
public class SensorTool : ResidenceToolBase
{
    ...
    // caption, and every RequestTool call and ResidenceWorkflow.StageOf lookup goes through it.
    ...
    // rail does, so every residence already on disk, all six samples, VariantDiff, VariantRevert and the
    ...
    // The coverage wash comes on with this tool and goes off with it. It answers "where can this residence
    ...
    private void Place()
    {
        ...
        // five-bedroom residence with forty of them impossible.
        Ctx.Controller.Select(ResidenceElementMarker.Kind.Sensor, sensor.id, reveal: false);
    ...
    public override void DrawRail()
    {
        ...
        if (cat == null)
        {
            ...
            Debug.LogWarning("SensorTool: no SensorCatalog is wired to the ResidenceRenderer, so the device "
                           + "grid is empty. Assign Assets/Resources/SensorCatalog.asset in ResidenceViz.unity.");
            return;
        }
    ...
    // A worn or personal item has TWO residences, and this row is where you pick. §4.5.1 for the pendant,
    // it is on the person, wherever they are, and the same is true of a key turner, which lives in a
    // pocket. But a residence is laid out long before anyone is named, and while a resident was the only
    ...
    // is: the point is that a motion sensor and a door sensor do NOT see the same amount of a residence.
