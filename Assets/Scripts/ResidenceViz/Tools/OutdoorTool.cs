...
// The one outdoor tool. Reachable only from the Outdoors stage, which only exists once a residence has
...
// nothing in ResidenceViz could author either, so ExteriorBridge.HasContent was permanently false and the
...
public class OutdoorTool : ResidenceToolBase
{
    public override string Id => "outdoor";
    public override string DisplayName => "Outdoors";

    public override string Hint =>
        "Draw walkways, entry ramps and railings around the residence. Click to place points; Enter finishes "
    ...
    private void DrawRailingControls()
    {
        ...
        if (palette == null || palette.entries == null || palette.entries.Count == 0)
        {
            ...
            Debug.LogWarning("OutdoorTool: no FencePalette entries are wired, so railings will not "
                           ...
                           + "Exterior/WorldRenderer in ResidenceViz.unity.");
        }
        else
        {
    ...
    private void DrawPathMaterialChips()
    {
        ...
        if (palette == null || palette.entries == null || palette.entries.Count == 0)
        {
            Debug.LogWarning("OutdoorTool: no PathMaterialPalette entries are wired, so paths will not "
                           ...
                           + "the Exterior/WorldRenderer in ResidenceViz.unity.");
            return;
        }
