...
public class WallTool : ResidenceToolBase
{
    ...
    public override void Enter(ResidenceToolContext ctx)
    {
        ...
        _chain.Clear();
        _typed = "";
        _thickness = ctx?.Level?.wallThickness ?? ResidenceConventions.DEFAULT_WALL_THICKNESS;
        _height = ctx?.Level?.ceilingHeight ?? ResidenceConventions.DEFAULT_CEILING_HEIGHT;
    }
    ...
    private void CaptureTypedDigits()
    {
        ...
        if (kb == null || _chain.Count == 0 || ResidenceEditController.TypingInUI) return;
