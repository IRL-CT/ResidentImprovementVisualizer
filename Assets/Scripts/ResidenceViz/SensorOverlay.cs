...
// identical reason: ResidenceEditController gates IResidenceTool.DrawOverlay on !PointerOverUI, so an overlay
...
//
// Everything is re-derived per frame rather than cached. Undo restores the whole ResidenceDoc without
...
public class SensorOverlay
{
    ...
    public void Draw(Camera cam, LevelDef level, VariantDef variant, string selectedId)
    {
        ...
        foreach (var exit in SensorCoverage.UnmonitoredExits(level))
        {
            ...
            Vector2 at = ResidenceMetrics.PointOnWall(wall, exit.offset);
