...
// ResidenceMetrics.NearestWall was lifted out of FurnitureTool when the wall-mount drag landed: placing
...
public static class MountPlacement
{
    ...
    /// </summary>
    public static bool WallCursor(ResidenceToolContext ctx, out Vector2 xz)
    {
        xz = Vector2.zero;
    ...
    /// <remarks>
    /// ResidenceConventions.MOUNT_REACH (1.2 m) is deliberately generous. It has to cover half a wall
    ...
    /// </remarks>
    public static WallDef Hover(ResidenceToolContext ctx, Vector2 cursor, out float offset, out int side)
    {
        offset = 0f;
        side = WallSide.Left;
        if (ctx?.Level == null) return null;

        return ResidenceMetrics.NearestWall(cursor, ctx.Level.walls, ResidenceConventions.MOUNT_REACH,
                                       out offset, out side);
    }
    ...
    /// </remarks>
    public static bool Place(ResidenceToolContext ctx, FurnitureCatalog.Entry entry,
                             WallDef wall, float offset, int side)
    {
        ...
        ctx.Controller.Select(ResidenceElementMarker.Kind.WallMount, mount.instanceId, reveal: false);
    ...
    public static Vector2 Ghost(FurnitureCatalog.Entry entry, float offset, WallDef wall, LevelDef level)
        => ResidenceMetrics.PointOnWall(wall, Fit(entry, offset, wall, level).offset);
}
