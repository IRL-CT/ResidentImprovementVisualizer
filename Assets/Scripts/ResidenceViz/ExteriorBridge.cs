...
// Adapts a ResidenceViz variant's optional outdoor layer into the shape the existing WorldRenderer expects.
//
...
// Buildings are deliberately left empty: the house itself is ResidenceRenderer's job, and handing
...
public static class ExteriorBridge
{
    ...
    /// </summary>
    public static EnvironmentDef ToEnvironmentDef(ResidenceDoc doc, VariantDef variant)
    {
        if (doc == null || variant == null || !HasContent(variant)) return null;

        SiteDef site = variant.exterior ?? new SiteDef();

        // A residence lot, not a 200 m industrial site: default the ground to something house-sized so an
        ...
        return new EnvironmentDef
        {
            ...
            id = "residenceviz:" + doc.id + ":" + variant.id,
            name = doc.name + " · " + variant.name,
            version = doc.version,
            tags = new List<string> { "residenceviz-exterior" },
            locked = false,
            site = site,
            buildingInstances = new List<BuildingInstance>(),   // the house is ResidenceRenderer's job
    ...
    /// Called the first time a user turns the exterior on for a residence.
    /// </summary>
    public static SiteDef NewExterior()
        => new SiteDef
        {
            ...
            scaleNote = "Residential lot around the residence.",
            maxGradeHeight = 4f,          // a garden grade, not a site-scale landform
            lotBoundary = null,
            outsideTerrainType = "grass", // not "water": a residence's surroundings are not a river
        };
}
