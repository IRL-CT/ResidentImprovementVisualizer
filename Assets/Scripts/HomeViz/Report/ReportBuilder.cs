...
//
// TURNING CIRCLES ARE NOT REPORTED HERE. They were, per room and as a whole-home count, and they came
...
namespace CXRHomeViz.Report
{
    public static class ReportBuilder
    {
        ...
        public const string OverviewSection = "The home";
        ...
        public static ReportDoc Build(HomeDoc doc, VariantDef from, VariantDef to,
                                      List<VariantDiff.Change> changes)
        {
            var report = new ReportDoc
            {
                homeName = doc?.name ?? "Home",
            ...
            plan.metrics.AddRange(WholeHomeMetrics(from, to));
                                                       ...
                                                       List<VariantDiff.Change> changes)
        {
            ...
            section.metrics.Add(new MetricRow
            {
                ...
                before = Percent(SensorCoverage.WholeHomeCoverage(from)),
                after = Percent(SensorCoverage.WholeHomeCoverage(to)),
                improved = SensorCoverage.WholeHomeCoverage(to)
                         > SensorCoverage.WholeHomeCoverage(from),
            });
        ...
        // "wall-mounteds fitting" and "smarts home device". A table for the exceptions is honest about
        ...
        private static List<MetricRow> WholeHomeMetrics(VariantDef from, VariantDef to)
        {
            ...
            // whole home, and both are the sentence a funder asks for, so both are summed over every
            // story. A stairs-only upper floor is exactly the home where the ground-floor count on
        ...
        private static List<MetricRow> RoomMetrics(LevelDef from, LevelDef to, RoomDef room)
        {
            ...
            float areaAfter = HomeMetrics.RoomArea(room);
            float areaBefore = before != null ? HomeMetrics.RoomArea(before) : 0f;
            rows.Add(new MetricRow
            {
                label = "Floor area",
            ...
            {
                float wAfter = openingAfter != null ? HomeMetrics.ClearWidth(openingAfter) : 0f;
                float wBefore = openingBefore != null ? HomeMetrics.ClearWidth(openingBefore) : 0f;
                rows.Add(new MetricRow
                {
                ...
                rows.Add(new MetricRow
                {
                    ...
                    improved = openingAfter != null && !HomeMetrics.HasThreshold(openingAfter)
                               && openingBefore != null && HomeMetrics.HasThreshold(openingBefore),
                });
            }
            return rows;
        }
        ...
        private static string Threshold(OpeningDef o)
            => HomeMetrics.HasThreshold(o) ? Units.Format(o.thresholdHeight) + " step" : "Step-free";
        ...
        private static OpeningDef NarrowestInto(LevelDef level, RoomDef room)
        {
            ...
            foreach (var o in level.openings)
            {
                ...
                Vector2 p = HomeMetrics.PointOnWall(wall, o.offset);
                if (DistanceToBoundary(p, poly) > 0.35f) continue;   // ~ half a wall + slop

                float w = HomeMetrics.ClearWidth(o);
        ...
        // Distance to the polygon's outline, inside or out. HomeMetrics.SignedDistanceInside answers
        ...
        private static float DistanceToBoundary(Vector2 p, IReadOnlyList<Vector2> poly)
        {
            ...
            for (int i = 0; i < poly.Count; i++)
                best = Mathf.Min(best, HomeMetrics.PointSegmentDistance(p, poly[i], poly[(i + 1) % poly.Count]));
            return best;
        }
        ...
        private static void CountOpenings(LevelDef level, out int stepFree, out int wide, out int total)
        {
            ...
            foreach (var o in level.openings)
            {
                ...
                total++;
                if (!HomeMetrics.HasThreshold(o)) stepFree++;
                if (HomeMetrics.ClearWidth(o) >= 0.8128f) wide++;   // 32 inches
            }
        }
        ...
        public static RoomDef RoomAt(LevelDef level, Vector2 p)
        {
            ...
            foreach (var room in level.rooms)
            {
                ...
                if (HomeMetrics.PointInPolygon(p, poly)) return room;
            }
            return null;
        }
