...
namespace CXRHomeViz.Report
{
    public static class HtmlReportWriter
    {
        ...
        public static string Write(ReportDoc report, HomeDoc doc, VariantDef to, out string error)
        {
            error = null;
            try
            {
                string dir = System.IO.Path.Combine(HomeStore.RootDir, DIR);
                System.IO.Directory.CreateDirectory(dir);

                string name = Sanitize(doc?.name ?? "Home") + " - " + Sanitize(to?.name ?? "Proposal")
                              + " - " + System.DateTime.Now.ToString("yyyy-MM-dd") + ".html";
                string path = System.IO.Path.Combine(dir, name);

                // Temp file then replace, the same discipline HomeStore.WriteAtomic uses: a report
        ...
        public static string Render(ReportDoc r)
        {
            ...
            sb.Append("<title>").Append(Esc(r.homeName)).Append(": ").Append(Esc(r.toName))
              .Append("</title>\n<style>\n").Append(Css()).Append("</style></head><body>\n");

            sb.Append("<header class=\"cover\">\n");
            sb.Append("<h1>").Append(Esc(r.homeName)).Append("</h1>\n");
            ...
            sb.Append("<footer><p>CXRHomeViz. Measurements come from the model."
                      + "</p></footer>\n");
            sb.Append("</body></html>\n");
            return sb.ToString();
        }
