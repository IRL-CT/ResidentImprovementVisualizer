...
// Photographing the same home twice, from the same place, once as it is and once as proposed.
//
...
//    home. A separate camera never sees any of it.
//
...
//    the reason its jobs are queued. The rail's button sets a flag; HomeEditController.Update drains
...
namespace CXRHomeViz.Report
{
    public class ReportCapture : MonoBehaviour
    {
        ...
        /// </summary>
        public static void Run(HomeEditController controller, HomeRenderer renderer, HomeDoc doc,
        ...
        private IEnumerator Capture(HomeEditController controller, HomeRenderer renderer, HomeDoc doc,
                                    string fromId, string toId, System.Action<string> status)
        {
            ...
            var from = HomeStore.FindVariant(doc, fromId);
            var to = HomeStore.FindVariant(doc, toId);
            ...
            // sixteen. On the single-story home this is two rebuilds, exactly as it always was.
            for (int i = 0; i < shots.Count; i++) { before.Add(null); after.Add(null); }

            foreach (int levelIndex in StoriesIn(shots))
            {
                renderer.RenderHome(doc, fromId, levelIndex);
            ...
            foreach (int levelIndex in StoriesIn(shots))
            {
                renderer.RenderHome(doc, toId, levelIndex);
                yield return null;
                for (int i = 0; i < shots.Count; i++)
            ...
            renderer.RenderHome(doc, restoreVariant);
        ...
        private List<Shot> Framings(VariantDef from, VariantDef to, List<VariantDiff.Change> changes)
        {
            ...
            Bounds home = Union(AllLevelBounds(from), AllLevelBounds(to));

            shots.Add(Plan(home, 0));
            shots.Add(Overview(home, 0));
