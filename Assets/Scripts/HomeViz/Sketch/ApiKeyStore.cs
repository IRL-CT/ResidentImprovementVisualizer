...
//
// NOT IN settings.json. HomeSettings is serialized wholesale by HomeStore.SaveSettings, it holds
...
//
// NOT IN THE EXPORT, by construction rather than by care. HomeStore.ExportHome zips home.json plus
// each underlay image and nothing else, so a file sitting in RootDir cannot ride along in a
// .homeviz archive. It is not in HomeDoc, so it is not in the undo snapshots either.
//
...
public static class ApiKeyStore
{
    ...
    private static string Path_ => System.IO.Path.Combine(HomeStore.RootDir, FILE_NAME);
    ...
    public static bool Set(string key, out string error)
    {
        ...
        try
        {
            Directory.CreateDirectory(HomeStore.RootDir);
