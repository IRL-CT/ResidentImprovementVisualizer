...
//
// The command bar drives a WORKFLOW STAGE (HomeWorkflow), not a mode in the Site tool's sense. Site
// is a tool whose operator ranges freely over terrain, buildings and generation; HomeViz is used
...
public class HomeEditController : MonoBehaviour, EditHistory.IHost
{
    ...
    /// <summary>
    /// The story being edited. Every tool sees it through HomeToolContext.Level, and the ground-plane
    ...
    ///
    /// It was hardcoded to levels[0]. HomeRenderer has taken a level index since it was written and
    ...
    /// </param>
    public void Select(HomeElementMarker.Kind kind, string id, bool reveal = true)
    {
        SelectedKind = kind;
        SelectedId = id;
        if (reveal && _stage != HomeStage.Select && !string.IsNullOrEmpty(id))
            RequestStage(HomeStage.Select);
    }
    ...
    private HomeStage _stage = HomeStage.Import;
    ...
    private HomeStage? _stageBefore;

    // The library rail folded down to a strip. Two rails cost 560 px of a 1280-wide window before the
    // plan gets any, and the library is consulted between homes rather than during one. Transient by
    ...
    // carry over from the home it was meant for to the next one.
    ...
    // EnvironmentScale as shared-by-design; the two knobs HomeViz needed (minHandleSize, Tick's
    ...
    private void Start()
    {
        ...
        SampleHomeInstaller.BackfillOccupants();
        ...
        SampleHomeInstaller.RefreshStaleSamples();
        SampleHomeInstaller.VerifyAgainstCatalog(homeRenderer?.Catalog);
        SampleHomeInstaller.VerifyAgainstCatalog(homeRenderer?.Sensors);
        SampleHomeInstaller.VerifyFloorFinishes(homeRenderer?.MaterialPalette);

        RefreshLibrary();
        SetStage(HomeStage.Import);
    ...
    /// also act on it.
    ///
    /// The whitelist is the load-bearing part. RoomMeshBuilder gives every floor a HomeElementMarker,
    ...
    private void TryAutoSelect()
    {
        ...
        if (PointerOverUI || _stage == HomeStage.Select) return;
        if (_active != null && _active.ClaimsClicks) return;
        if (GizmoBusy) return;
        if (Mouse.current == null || !Mouse.current.leftButton.wasPressedThisFrame) return;

        if (!_ctx.PickElement(out HomeElementMarker marker, out _)) return;
    ...
    private static bool IsInspectable(HomeElementMarker.Kind kind)
        => kind == HomeElementMarker.Kind.Furniture
        || kind == HomeElementMarker.Kind.WallMount
        ...
        || kind == HomeElementMarker.Kind.Sensor
        || kind == HomeElementMarker.Kind.Occupant;
    ...
    // they were written; HomeEditController never did, and the gap was survivable only because the
    ...
    private void HandleGlobalKeys()
    {
        ...
        if (kb.escapeKey.wasPressedThisFrame)
        {
            ...
            else if (_stage == HomeStage.Select && _stageBefore.HasValue) RequestStage(_stageBefore.Value);
        }
    ...
    private void TickGizmo()
    {
        ...
        // header: Rebuild() destroys and respawns every GameObject, and undo swaps the whole HomeDoc
        // without telling anyone, so a held reference is a stale object pointing at nothing.
        var go = homeRenderer != null ? homeRenderer.GetGO(item.instanceId) : null;
        if (go == null)
        {
            if (_gizmo.enabled) { _gizmo.enabled = false; _gizmo.Clear(); _gizmoTarget = null; }
            return;
    ...
    {
        if (SelectedKind != HomeElementMarker.Kind.Furniture || string.IsNullOrEmpty(SelectedId)) return null;
    ...
    private void OnGizmoMove(Vector3 delta)
    {
        ...
        homeRenderer?.PoseFurnitureGO(item);
    }
    ...
    private void OnGizmoRotate(Vector3 eulerDelta)
    {
        ...
        // HomeMetrics, SelectionOverlay, the occupancy checks. Is computed from rotationY alone, so an
    ...
    private void OnGizmoResize(float delta)
    {
        ...
        // multiplier. HomeViz has no such field in play. boxSizeMeters is the item's true size and is
    ...
    /// document dirty, and rebuild the furniture: not the whole home, which did not change.
    /// </summary>
    ...
    public void RefitFurniture(ObjectInstance item)
    {
        ...
        var entry = homeRenderer?.Catalog?.Get(item.prefabType);
    ...
    private void AfterOpen()
    {
        ...
        ClearSelection();
        _confirmReset = false;
        _levelIndex = 0;   // a home always opens on its ground floor
        RefreshLibrary();
        SyncStages();   // this home may or may not have an exterior; the command bar follows it

        homeRenderer?.RenderHome(Doc, Doc.activeVariantId, LevelIndex);
    ...
    public void SetActiveVariant(string variantId)
    {
        ...
        homeRenderer?.RenderHome(Doc, variantId, LevelIndex);
        MarkDirty();
    }

    /// <summary>Queues a variant switch for the next frame. Use this from anything drawn in OnGUI.</summary>
    // Switching tears down and rebuilds every GameObject in the home and clears the undo stack, so
    ...
    public void SetActiveLevel(int index)
    {
        ...
        ClearSelection();
        History.Clear();
        homeRenderer?.RenderHome(Doc, Doc.activeVariantId, index);
        viewController?.FrameContent();
        Status("Now on " + LevelName(index) + ".");
    }

    /// <summary>Deferred, like every other request drawn in OnGUI: this rebuilds the whole home.</summary>
    ...
    public string LevelName(int index)
    {
        ...
        if (levels == null || index < 0 || index >= levels.Count) return HomeStore.DefaultLevelName(index);
        var l = levels[index];
        return string.IsNullOrEmpty(l?.name) ? HomeStore.DefaultLevelName(index) : l.name;
    }

    /// <summary>
    /// Adds a story to every variant and switches to it. Deliberately NOT gated on the baseline being
    /// unlocked. See HomeStore.AddLevel for why an empty story asserts nothing about the home. What
    ...
    public void AddLevel(string name = null)
    {
        ...
        int index = HomeStore.AddLevel(Doc, string.IsNullOrWhiteSpace(name) ? null : name.Trim());
        MarkDirty();
        _levelIndex = index;
        ClearSelection();
        homeRenderer?.RenderHome(Doc, Doc.activeVariantId, index);
    ...
    public void RemoveLevel(int index)
    {
        ...
        if (!HomeStore.RemoveLevel(Doc, index, out string error)) { Status(error); return; }
    ...
    public void SetStage(HomeStage stage)
    {
        ...
        if (stage == HomeStage.Select) { if (_stage != HomeStage.Select) _stageBefore = _stage; }
        else _stageBefore = null;

        bool leavingPeople = _stage == HomeStage.People && stage != HomeStage.People;
        bool leavingReview = _stage == HomeStage.Review && stage != HomeStage.Review;
        _stage = stage;
        ...
        if (stage == HomeStage.People) TimelineExpanded = true;
        ...
        if (leavingReview) homeRenderer?.SetGhostVariant(null, false);
    ...
    {
        bool wantOutdoors = HomeWorkflow.OutdoorsUI && Doc != null && Doc.exteriorEnabled;
        ...
        if (!_stages.Contains(_stage)) RequestStage(HomeStage.Structure);
    }
    ...
    private void OnGUI()
    {
        ...
        if (Doc != null && !string.IsNullOrEmpty(SelectedId))
            _selectionView.Draw(cam, Level, Variant, SelectedKind, SelectedId, homeRenderer);
    ...
    {
        if (homeRenderer == null) return;

        homeRenderer.Occupancy.Playing = false;
        homeRenderer.Occupancy.ScrubTo(alert.minute);
        homeRenderer.UpdateOccupantPoses();
        homeRenderer.UpdateSensorStates();
        ...
        Select(HomeElementMarker.Kind.Sensor, alert.sensorId, reveal: false);
    ...
    {
        if (Doc == null || homeRenderer == null || _timelineRect.height <= 0f) return;

        _timeline.Draw(_timelineRect, TimelineExpanded, Variant, Level, homeRenderer.Occupancy,
                       homeRenderer.SensorDay);
        ...
        Select(HomeElementMarker.Kind.Occupant, clicked);
    ...
    {
        if (viewController == null || homeRenderer == null || string.IsNullOrEmpty(id)) return;
        ...
        var go = homeRenderer.GetGO(id);
        if (go == null || !go.activeInHierarchy)
        {
            Status(string.IsNullOrEmpty(label) ? "Nothing to focus on." : label + " is out of the home right now.");
            return;
        }
    ...
    private void DrawLeftRail()
    {
        ...
        if (_leftCollapsed)
        {
            ...
            UITheme.Tip($"Open the home library ({_library.Count})");
            UITheme.EndPanel();
            return;
        }
        ...
        if (UITheme.SecondaryButton("Import", GUILayout.Height(UITheme.PrimaryH))) ImportHome();
        UITheme.Tip("Open a .homeviz archive someone sent you");
        GUILayout.EndHorizontal();
        ...
        if (_library.Count == 0)
        {
            ...
            {
                NewHome();
                RequestStage(HomeStage.Import);
            }
            UITheme.Tip("No homes yet. This starts one and takes you straight to importing a plan "
        ...
        foreach (var row in _library)
        {
            ...
            // The version and option count are the row's own content: what tells two saved homes
            ...
            UITheme.Tip(isOpen ? "The home you have open" : $"Open {row.name}");
            if (open) OpenHome(row.id);
        }
        UITheme.EndScroll();
        ...
        if (Doc != null)
        {
            ...
            Doc.name = UITheme.TextRow("Name", Doc.name, "What this home is called in the library");
            ...
            UITheme.Tip("Write a .homeviz archive holding this home and its sketch, to send to "
                        + "another machine");
            bool star = UITheme.GhostButton(Doc.favorite ? "Unstar" : "Star");
            UITheme.Tip(Doc.favorite ? "Stop starring this home" : "Star this home in the library");
            if (star)
            {
            ...
            UITheme.Tip("Move this home out of the library. Nothing is destroyed. It goes to the "
    ...
    // Rebuilds this home from the sample it came from. Startup already refreshes every sample nobody
    // has touched (see SampleRefresh), so the only homes that ever reach this button are ones with
    ...
    private void DrawResetToSample()
    {
        ...
        if (SampleHomeInstaller.ResolveKey(Doc) == null) return;
        ...
        if (!_confirmReset)
        {
            ...
            UITheme.Tip("Rebuild this home from the shipped plan, picking up any improvement made to "
        ...
        if (UITheme.DangerButton(proposals > 0
            ...
            ResetToSample();
        UITheme.Tip("Rebuild this home from the shipped plan. Every proposal, and any edit to the "
                    + "baseline, is thrown away. This cannot be undone.");
        UITheme.GapTight();
        if (UITheme.GhostButton("Cancel")) _confirmReset = false;
        UITheme.Tip("Leave this home as it is");
    }
    ...
    private void ResetToSample()
    {
        ...
        if (!SampleHomeInstaller.ResetToSample(Doc, out string error))
        {
    ...
    private void DrawTopBar()
    {
        ...
        var labels = Doc != null ? HomeWorkflow.LabelsFor(_stages) : null;
        ...
        // The floor chip appears ONLY on a home with more than one story: the same
        // conditional-visibility rule the Outdoors stage follows, and the reason every existing
        // single-story home's command bar is untouched by this. Its width joins the MEASURED reserve
        ...
        // every home that has ever existed would be a permanent control that can do nothing.
        //
        // Deferred like the stage picker, and for the stronger version of the same reason: switching
        // story tears down and rebuilds every GameObject in the home and clears the selection, so
        ...
        //
        // HomeSettings.metricUnits has existed, been persisted, and been wired to Units.Display through
        // HomeStore.ApplySettings the entire time; it simply had no control, so the only way to switch
        ...
        {
            bool metric = HomeStore.Settings.metricUnits;
            if (UITheme.Chip(metric ? MetricLabel : ImperialLabel, false, GUILayout.Width(unitsW)))
            {
                HomeStore.Settings.metricUnits = !metric;
                HomeStore.SaveSettings();
            }
    ...
    private void DrawRightRail()
    {
        ...
        if (Doc == null)
        {
            ...
            UITheme.Tip("Create a new home, or open one from the library on the left, to begin.");
            UITheme.EndPanel();
            return;
        }
    ...
    private void DrawModeBand()
    {
        ...
        if (_modeBand.CompareRequested) RequestStage(HomeStage.Review);
        ...
        {
            var doomed = HomeStore.FindVariant(Doc, _modeBand.DeleteVariantId);
    ...
    private void NewProposalFrom(VariantDef source)
    {
        ...
        copy.name = HomeStore.NewProposalName(Doc, System.DateTime.Now);
    ...
    private void DrawExteriorToggle()
    {
        bool on = UITheme.Toggle("Outdoor additions", Doc.exteriorEnabled,
            "Turn on for a home where an outdoor change is part of the proposal: an entry ramp, "
    ...
    {
        if (Doc == null || homeRenderer == null) return;
        ...
        SaveHome();
        ReportCapture.Run(this, homeRenderer, Doc, baseline.id, cur.id, Status);
    }
    ...
    // Undo and redo restore the whole HomeDoc, which can delete the very wall the active tool is
