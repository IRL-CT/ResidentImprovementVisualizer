...
public class HomeRenderer : MonoBehaviour
{
    ...
    // anyway, because tinting a shared one would light every door sensor in the home at once.
    ...
    // an edit routed through here without it leaves the ghost describing the home as it was before the
    ...
    // things that still need it: HomeEditController.FocusElement resolves ids through _byId, so F and
    ...
    /// calling <see cref="Rebuild"/>, destroys and recreates every GameObject in the home each frame,
    ...
    /// which destroys and respawns every GameObject in the home (including the one being dragged) 
    ...
    private void PoseMount(GameObject go, WallMountDef mount, Vector3 size)
    {
        ...
        var marker = go.GetComponent<HomeElementMarker>();
    ...
    // it at their current position would make it slide around the home every simulated minute while
    ...
    private void RenderSensors()
    {
        ...
        foreach (var sensor in _level.sensors)
        {
            ...
            Mark(view.go, HomeElementMarker.Kind.Sensor, sensor.id, sensor.hostId);
    ...
    /// its offset in the rail, must not destroy and rebuild every GameObject in the home per frame.
    /// </summary>
    ...
    private void RenderOccupants()
    {
        ...
        foreach (var person in _variant.occupants)
        {
            ...
            Mark(view.go, HomeElementMarker.Kind.Occupant, person.id, null);
    ...
    private OccupantView BuildOccupantMarker(OccupantDef person, int index)
    {
        ...
        float height = seated ? HomeConventions.EYE_HEIGHT_SEATED : 1.70f;
        float width = 0.45f;
    ...
    ///
    /// Most colliders in a HomeViz scene are not physics at all. An opening has no geometry, furniture
    /// renders as a labeled massing box, an occupant is a tinted capsule; each carries a collider
    /// purely so HomeToolContext.PickElement's raycast can find it. Left solid, they are what makes the
    ...
    /// query in HomeViz). What stays solid is the shell (wall meshes, floors, ceilings) which is the
    ...
    /// and getting that wrong leaves a home permanently green. Building both halves into _ghostRoot
    ...
    private void BuildGhost()
    {
        ...
        var other = HomeStore.FindVariant(_doc, _ghostVariantId);
