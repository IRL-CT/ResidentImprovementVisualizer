...
// Camera behaviour for the two ways of looking at a home.
//
...
public class ViewController : MonoBehaviour
{
    ...
    // rail. See BeginDrag.
    [SerializeField] private HomeEditController editController;
    ...
    private void Awake()
    {
        ...
        if (editController == null) editController = FindFirstObjectByType<HomeEditController>();
    }
    ...
    /// keeps going regardless, which is what HomeEditController's gizmo note depends on.</summary>
    ...
    private bool TypingInUI
        => editController != null ? HomeEditController.TypingInUI : GUIUtility.keyboardControl != 0;
    ...
    private float Elevation => homeRenderer?.Level?.elevation ?? 0f;
    ...
    /// <summary>
    /// Somewhere you can actually stand. The orbit pivot is the framing center of the home, which is
    ...
    /// ignores furniture (HomeMetrics.cs:195), which is exactly right here. Furniture no longer
    ...
    private Vector3 StandableStart()
    {
        ...
        var room = HomeMetrics.RoomAt(new Vector2(_pivot.x, _pivot.z), level);
        if (room == null)
        {
            float bestArea = 0f;
            foreach (var r in level.rooms)
            {
                ...
                float area = HomeMetrics.RoomArea(r);
        ...
        var circle = HomeMetrics.LargestInscribedCircle(room);
