...
public static class HomeStore
{
    ...
    // ---------------------------------------------------------------------------------------
    // Migration off the old product name
    // ---------------------------------------------------------------------------------------

    // The Unity productName was CXRBrownfield until the terminology rename, and persistentDataPath is
    // DERIVED from it (<...>/LocalLow/<company>/<productName>). Changing the name therefore moves the
    // whole library out from under RootDir: every home, underlay and settings.json is still on disk
    // but at an address nothing looks at any more. Move the tree once instead.
    //
    ...
    private const string LEGACY_PRODUCT_NAME = "CXRBrownfield";

    static HomeStore() => MigrateLegacyRoot();
    ...
    private static void MigrateLegacyRoot()
    {
        try
        {
            ...
            string legacyRoot = Path.Combine(companyDir, LEGACY_PRODUCT_NAME, "CXRHomeViz");
            ...
            Debug.Log($"[HomeStore] Moved the library from the previous product folder: {legacyRoot} → {RootDir}");
        }
        catch (Exception e)
        {
            ...
            Debug.LogWarning($"[HomeStore] Could not move the library off the previous product folder: {e.Message}");
        }
    }
    ...
    /// "Proposal 2 08/24/2026", "Proposal 3 …": the ordinal is the home's overall proposal count
    /// (baseline excluded), the date is the day it was made. Uniqued against the home's existing
    ...
    /// </summary>
    public static string NewProposalName(HomeDoc doc, DateTime when)
    {
    ...
    /// leave a home pointing at a sketch that never existed.
    /// </summary>
    public static string ImportUnderlayBytes(string homeId, byte[] bytes, string desiredName,
                                             out string error)
    {
        error = null;
        try
        ...
        {
            error = e.Message;
            Debug.LogError($"[HomeStore] Underlay page write failed: {e}");
            return null;
        }
    }
    ...
    public static HomeDoc ImportHome(string zipPath, out string error)
    {
        ...
        try
        {
            ...
            // indistinguishable from correct while a home could only have one sketch and is simply
            // wrong the moment a two-story home is shared: both floors would end up pointing at
    ...
    public class HomeSettings
    {
        ...
        public bool showExteriorLayer;      // top-level UI toggle; per-home data still gates rendering
        public string lastOpenedHomeId;
        // Set once SampleHomeInstaller has given already-seeded samples the household they now ship with.
    ...
    public static UnderlayDef UnderlayFor(HomeDoc doc, string levelId) => Stories.UnderlayFor(doc, levelId);

    public static void SetUnderlay(HomeDoc doc, string levelId, UnderlayDef underlay)
        => Stories.SetUnderlay(doc, levelId, underlay);

    public static bool HasAnyUnderlay(HomeDoc doc) => Stories.HasAnyUnderlay(doc);

    public static int AddLevel(HomeDoc doc, string name = null) => Stories.Add(doc, name);

    public static bool RemoveLevel(HomeDoc doc, int index, out string error)
        => Stories.Remove(doc, index, out error);

    public static void RenameLevel(HomeDoc doc, string levelId, string name)
