...
//   * Windows x64 only, which is the only platform HomeViz builds for (Build -> HomeViz (PC, Windows)).
...
//     (The one place in HomeViz that does reach the network is SketchPlanGenerator, which is opt-in
