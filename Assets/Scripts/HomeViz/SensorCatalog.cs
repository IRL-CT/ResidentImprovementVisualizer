...
// GUID reference in HomeViz.unity for a word nobody sees. What the class name has to be right about is
...
// cannot reference a ScriptableObject; SampleHomeInstaller.VerifyAgainstCatalog compares the two on
...
public class SensorCatalog : ScriptableObject
{
    ...
    public class Entry
    {
        [Tooltip("Catalog key. Matches a PrefabRegistry key when real art exists, and the id in " +
                 "SensorDevices.cs, which SampleHomeInstaller re-checks on seed.")]
        public string id;
    ...
    /// .EffectiveRules reads the defaults, so a home that never touched a threshold picks up an
    /// improved one, and a home that did keeps its own.
    /// </remarks>
