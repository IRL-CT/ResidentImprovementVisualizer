...
// read, and a read that marks a home unsaved is a bug people learn to distrust the app for.
public class MonitorTool : HomeToolBase
{
    ...
    private HomeRenderer Renderer => Ctx?.Renderer;
    ...
    public override void DrawRail()
    {
        ...
        if (level.sensors == null || level.sensors.Count == 0)
        {
            ...
            UITheme.Tip("Nothing is watching this home yet, so there is nothing for a caregiver to "
    ...
    private void DrawDayPicker()
    {
        ...
        int next = UITheme.Segmented("Day", mode,
            ...
            new[]
            {
                ...
                "The same day with the report's own scenarios acted out on this home's devices: the "
    ...
    private void DrawAlertCard(SensorAlert alert)
    {
        ...
        UITheme.Tip("Look at the entry camera. It is the only one in the home. §4.5.2.");
    ...
    private void Focus(SensorAlert alert)
    {
        ...
        Ctx.Controller.Select(HomeElementMarker.Kind.Sensor, alert.sensorId, reveal: false);
    ...
    private bool CanSee(SensorAlert alert)
    {
        ...
        var sensor = HomeRenderer.FindSensor(alert.sensorId, Ctx.Level);
    ...
    private void DrawResidents()
    {
        ...
        foreach (var person in roster)
        {
            ...
            if (_role == Role.Dsp && poses != null && poses.TryGetValue(person.id, out var pose))
                ...
                where = HasPendant(person) ? "Pendant on" : "At home";
    ...
    private void DrawFoldouts()
    {
        ...
        if (_showCoverage)
        {
            // Whole BUILDING, not the story on screen. These read as claims about the home, and the
            ...
            float whole = SensorCoverage.WholeHomeCoverage(variant);
            UITheme.Value("Coverage", $"{whole * 100f:0}%",
                          "The share of this home's floor a movement sensor can see, across every "
    ...
    private void SelectFirst(LevelDef level, string deviceType)
    {
        foreach (var s in level.sensors)
        {
            ...
            Ctx.Controller.Select(HomeElementMarker.Kind.Sensor, s.id, reveal: false);
    ...
    public override void DrawOverlay()
    {
        ...
        // Alert pins in the plan, so the console and the home agree about where something is
