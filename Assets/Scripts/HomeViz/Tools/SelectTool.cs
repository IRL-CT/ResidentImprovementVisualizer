...
public class SelectTool : HomeToolBase
{
    ...
    public override void HandleInput()
    {
        ...
        if (LeftClicked() && !gizmo)
        {
            if (Ctx.PickElement(out HomeElementMarker marker, out _))
            {
                ...
                // marker along the ray and stops, and HomeRenderer builds the opening handle proud of
                ...
                bool opening = marker.kind == HomeElementMarker.Kind.Opening;
                ...
                {
                    Ctx.Controller.SelectedKind = opening ? HomeElementMarker.Kind.Wall : marker.kind;
    ...
    {
        // Arrows and Z/X are letters while a rail field has the caret. See HomeEditController.TypingInUI.
        if (HomeEditController.TypingInUI) return;

        var item = FindFurniture(Ctx.Controller.SelectedId);
        if (item == null || Ctx.Controller.SelectedKind != HomeElementMarker.Kind.Furniture) return;
    ...
    //  - THE CURSOR'S FLOOR PROJECTION IS NOT WHERE THE ITEM IS. HomeRenderer draws a mount at
    ...
    {
        if (Ctx.Controller.SelectedKind != HomeElementMarker.Kind.WallMount) { EndMountDrag(); return; }
        ...
        if (LeftClicked())
        {
            if (!Ctx.PickElement(out HomeElementMarker hit, out _)
                || hit.kind != HomeElementMarker.Kind.WallMount || hit.id != m.instanceId) return;
        ...
        var wall = HomeMetrics.NearestWall(at + _grabOffset, Ctx.Level.walls, HomeConventions.MOUNT_REACH,
        ...
        // Re-pose the item, do not rebuild the home: Ctx.Changed() means a full Rebuild(), which
    ...
    /// HomeRenderer.RenderWallMounts poses the item with: two copies would be two chances to disagree
    ...
    // early when the variant is locked, and locked is the DEFAULT state of every home in the library,
    ...
    public override void DrawRail()
    {
        ...
        {
            case HomeElementMarker.Kind.Wall: summary = DrawWall(id); break;
            case HomeElementMarker.Kind.Opening: summary = DrawOpening(id); break;
            case HomeElementMarker.Kind.Room:
            case HomeElementMarker.Kind.Floor:
            case HomeElementMarker.Kind.Ceiling: summary = DrawRoom(id); break;
            case HomeElementMarker.Kind.Furniture: summary = DrawFurniture(id); break;
            case HomeElementMarker.Kind.WallMount: summary = DrawWallMount(id); break;
            case HomeElementMarker.Kind.Sensor: summary = DrawSensor(id); break;
            case HomeElementMarker.Kind.Occupant: DrawOccupant(id); break;
        }
    ...
    private (string, string) DrawWall(string id)
    {
        ...
        var summary = ($"{Units.Format(HomeMetrics.WallLength(w))} long · "
                       + $"{Units.Format(t)} thick · {Units.Format(h)} high",
                       "Length, thickness and height of this wall");

        if (Ctx.IsLocked)
        {
            // Browsable on a LOCKED baseline too, which is the default state of every home in the
    ...
    private void DrawOpeningList(WallDef w)
    {
        ...
        foreach (var o in list)
        {
            ...
            bool selected = Ctx.Controller.SelectedKind == HomeElementMarker.Kind.Opening
                            && Ctx.Controller.SelectedId == o.id;

            if (UITheme.StateRow(SensorPose.OpeningLabel(o, Ctx.Level), Units.Format(o.width), selected))
                Ctx.Controller.Select(HomeElementMarker.Kind.Opening, o.id, reveal: false);
    ...
    private (string, string) DrawOpening(string id)
    {
        ...
        bool step = HomeMetrics.HasThreshold(o);
        ...
        var summary = ($"{Units.Format(HomeMetrics.ClearWidth(o))} clear · "
                       + $"{Units.Format(o.width)} × {Units.Format(o.height)} rough{sill}{thresh}",
                       HomeMetrics.IsClearWidthMeasured(o)
        ...
        float maxW = Mathf.Max(HomeConventions.MIN_OPENING_WIDTH, OpeningFit.MaxWidth(o, wall, Ctx.Level));
        float nw = MeasureUI.Length("Width", "The rough opening. Drag it, or type a measured width.",
                                    o.width, 0.0127f, HomeConventions.MIN_OPENING_WIDTH, maxW);
        ...
        float maxH = Mathf.Max(HomeConventions.MIN_OPENING_HEIGHT, wallH - o.sillHeight);
        float nh = MeasureUI.Length("Height", "Opening height", o.height, 0.025f,
                                    HomeConventions.MIN_OPENING_HEIGHT, maxH);
    ...
    private void SetWidth(OpeningDef o, float meters)
    {
        ...
        // Idempotent until the central EndGesture that HomeEditController.Update runs on every
    ...
    private (string, string) DrawRoom(string id)
    {
        ...
        var summary = (Units.FormatArea(HomeMetrics.RoomArea(r)), "Floor area of this room");
    ...
    // HomeRenderer draws, what FurnitureFit tests against a doorway and what the occupancy checks
    ...
    private float DimensionSlider(string label, string tooltip, float meters, float nominal,
                                  ref bool changed)
    {
        float lo = Mathf.Max(HomeEditController.MIN_ITEM_SIZE, 0.5f * nominal);
    ...
    private (string, string) DrawWallMount(string id)
    {
        ...
        float len = HomeMetrics.WallLength(host);
    ...
    private (string, string) DrawSensor(string id)
    {
        ...
        // consequential thing about a device in a home someone lives in, and "Not connected" is as
        ...
        bool monitored = UITheme.Toggle("Reports to staff", s.monitored,
            "Off, it still senses and still prompts in the home. It simply raises nothing "
    ...
    // threshold", and a home whose stove alert keeps crying wolf needs to move it without moving it
    // for every other home, which is exactly what SensorDef.rules is and what these write into.
    private void DrawSensorRules(SensorDef s)
    {
        ...
        for (int i = 0; i < rules.Count; i++)
        {
            ...
            // every home. The first edit is what gives this device rules of its own.
    ...
    private SensorDef FindSensor(string id) => HomeRenderer.FindSensor(id, Ctx.Level);
    ...
    private void DrawOccupant(string id)
    {
        ...
        if (UITheme.SecondaryButton("Edit this person's day"))
            Ctx.Controller.RequestStage(HomeStage.People);
    ...
    private void DeleteSelected()
    {
        ...
        switch (Ctx.Controller.SelectedKind)
        {
            ...
            case HomeElementMarker.Kind.Sensor:
            ...
            case HomeElementMarker.Kind.Occupant:
    ...
    private static bool IsRoomKind(HomeElementMarker.Kind k)
        => k == HomeElementMarker.Kind.Room
        || k == HomeElementMarker.Kind.Floor
        || k == HomeElementMarker.Kind.Ceiling;
