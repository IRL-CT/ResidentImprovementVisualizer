...
public class CompareTool : HomeToolBase
{
    ...
    // restores the whole HomeDoc without telling anyone, so a held Change would describe an element
    ...
    private VariantDef From => HomeStore.Baseline(Ctx?.Doc);
    ...
    private VariantDef Resolve(string id)
        => string.IsNullOrEmpty(id) ? null : HomeStore.FindVariant(Ctx?.Doc, id);
    ...
    public override void DrawRail()
    {
        ...
        if (from == null)
        {
            UITheme.Glyph("⚠", "This home has no base environment.", UITheme.Ink3);
            return;
        }
        if (to == null)
        {
            UITheme.Value("Before", from.name ?? "(unnamed)", "The home as it stands.");
            UITheme.Gap();
    ...
    // across rebuilds, undo and variant switches by design (HomeRenderer.SetGhostVariant), and a
    ...
    private List<KeyValuePair<string, List<VariantDiff.Change>>> GroupByRoom(VariantDef to)
    {
        ...
        foreach (var change in _changes)
        {
            ...
            // single-story home is a word added to every row for no information.
    ...
    public static string RoomNameAt(LevelDef level, VariantDiff.Change change)
    {
        ...
        foreach (var room in level.rooms)
        {
            ...
            if (!HomeMetrics.PointInPolygon(change.worldPos, poly)) continue;
    ...
    // VariantDiff.ElementKind and HomeElementMarker.Kind are parallel by construction: the diff
    // reports on exactly the things the renderer marks.
    private static HomeElementMarker.Kind MarkerKind(VariantDiff.ElementKind kind) => kind switch
    {
        VariantDiff.ElementKind.Wall => HomeElementMarker.Kind.Wall,
        VariantDiff.ElementKind.Opening => HomeElementMarker.Kind.Opening,
        VariantDiff.ElementKind.Room => HomeElementMarker.Kind.Room,
        VariantDiff.ElementKind.Furniture => HomeElementMarker.Kind.Furniture,
        VariantDiff.ElementKind.WallMount => HomeElementMarker.Kind.WallMount,
        _ => HomeElementMarker.Kind.Occupant,
    };
}
