...
public class UnderlayTool : HomeToolBase
{
    ...
    public override void Tick()
    {
        ...
        if (_pendingRemovePlan != null)
        {
            ...
            HomeStore.SetUnderlay(Ctx.Doc, cleared, null);
    ...
    public override void DrawRail()
    {
        ...
        var underlay = HomeStore.UnderlayFor(Ctx.Doc, Ctx.Level?.id);
    ...
    /// <summary>
    /// The one control in HomeViz that reaches the network, and the only place that says so.
    ///
    ...
    private void DrawGenerate(UnderlayDef underlay)
    {
        ...
        // every home in the library and of all six samples, pressing the button was silent: no
        ...
        string tip = "The sketch image is sent to Anthropic and read by Claude, which returns the "
                   + "rooms, doors, windows and furniture it can see. Nothing else about this home "
    ...
    private void DrawApiKey()
    {
        ...
        if (source == ApiKeyStore.Origin.File)
        {
            ...
            UITheme.Tip(ApiKeyStore.Masked + ". Stored on this machine only, in plain text, and "
                      + "never part of an exported home.");
        ...
        UITheme.Glyph("⚠", "An Anthropic API key is needed to read a plan. It is stored on this "
                         + "machine only, in plain text, and is never part of an exported home.",
                      UITheme.Warn);
    ...
    private void BeginGeneration()
    {
        ...
        var underlay = HomeStore.UnderlayFor(Ctx.Doc, level0.id);
    ...
    /// </summary>
    private System.Collections.IEnumerator RunGeneration(SketchFrame frame, LevelDef level, string homeId, string levelId)
    {
        bool answered = false;

        yield return Ctx.Controller.StartCoroutine(SketchPlanGenerator.Run(
            _texture, frame, level.ceilingHeight, level.wallThickness,
            homeId, levelId, _genCall,
    ...
    /// <summary>
    /// Writes the plan into the story, deferred because it rebuilds every GameObject in the home.
    /// </summary>
    private void ApplyGeneration()
    {
        ...
        // A minute is long enough for somebody to switch home or floor while it ran. Applying a plan
        // to the wrong story would be a silent, destructive mistake, so it simply refuses.
        if (Ctx.Doc == null || Ctx.Doc.id != result.forHomeId || Ctx.Level?.id != result.forLevelId)
        {
    ...
    /// same act: importing a plan set IS how a home gets its floors. Each floor is ONE row:
    ...
    private void DrawFloors()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            var u = HomeStore.UnderlayFor(Ctx.Doc, levelId);
            ...
            if (lvl != null && typed != lvl.name)
            {
                ...
                HomeStore.RenameLevel(Ctx.Doc, levelId, typed);
                c.MarkDirty();
            ...
            if (showRemove)
            {
                ...
                // The removal itself stays deferred. It rebuilds every GameObject in the home.
                ...
                UITheme.Tip(count > 1
                    ...
                    : "A home has to have at least one floor, so this removes only its plan. "
        ...
        UITheme.Gap();
        string suggested = HomeStore.DefaultLevelName(count);
    ...
    // OnGUI-owned, like HomeEditController's _confirmReset: arming between passes redraws
    ...
    {
        string stored = HomeStore.ImportUnderlay(Ctx.Doc.id, sourcePath, out string err);
    ...
    private void AdoptSketch(string storedFileName, string sourceDocument, int sourcePage, string levelId)
    {
        ...
        HomeStore.SetUnderlay(Ctx.Doc, levelId, new UnderlayDef
        {
    ...
    private void ImportPage(PdfRaster.PdfDocument doc, string sourcePath, int page, float dpi, string levelId)
    {
        ...
        // A plain ASCII hyphen, because HomeStore.SanitizeFileName rewrites anything outside
        ...
        string stored = HomeStore.ImportUnderlayBytes(Ctx.Doc.id, png, name, out string err);
        if (stored == null) { Ctx.Controller.Status("Import failed: " + err); return; }
    ...
    /// page adds one above it, so importing a two-page set into a fresh home leaves it with exactly
    ...
    private void ImportAllPages()
    {
        ...
        for (int page = 1; page <= _pdf.PageCount; page++)
        {
            ...
            string stored = HomeStore.ImportUnderlayBytes(doc.id, png, $"{stem} - p{page}.png",
                                                          out string err);
            ...
            int levelIndex = page == 1 ? startIndex : HomeStore.AddLevel(doc);
            string levelId = LevelIdAt(levelIndex);
            if (levelId == null) continue;

            HomeStore.SetUnderlay(doc, levelId, new UnderlayDef
            {
    ...
    private void EnsureQuad()
    {
        ...
        {
            _loadError = "That sketch file is missing from this home's folder.";
