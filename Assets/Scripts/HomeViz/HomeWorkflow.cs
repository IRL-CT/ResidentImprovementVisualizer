...
//
// This is HomeViz's answer to the Site tool's UIShell/UIMode, minus the machinery. Site needs a
...
public static class HomeWorkflow
{
    ...
    private static readonly Dictionary<HomeStage, string[]> Tools = new Dictionary<HomeStage, string[]>
    {
        ...
        [HomeStage.Import]    = new[] { "underlay" },
        [HomeStage.Structure] = new[] { "wall", "opening", "room" },
        [HomeStage.Furnish]   = new[] { "furniture" },
        ...
        [HomeStage.SmartLiving] = new[] { "sensor", "monitor" },
        [HomeStage.People]    = new[] { "people" },
        ...
        [HomeStage.Review]    = new[] { "compare", "measure" },
        // Only reachable once the home's exterior layer is switched on. See VisibleStages.
        [HomeStage.Outdoors]  = new[] { "outdoor" },
    };

    /// <summary>
    /// What each tab does, shown on hover. This is where a stage's explanation lives now that no rail
    /// prints one: the active tool's own <see cref="IHomeTool.Hint"/> overrides the entry for the stage
    /// you are actually in (see HomeEditController.StageTips).
    /// </summary>
    private static readonly Dictionary<HomeStage, string> Tips = new Dictionary<HomeStage, string>
    {
        [HomeStage.Select]    = "Pick a wall, room or item to inspect it, and to move or resize "
                              + "furniture. A wall lists the openings in it.",
        [HomeStage.Import]    = "Import a floor plan and set its scale. Everything traced afterwards "
                              + "comes out at true size.",
        [HomeStage.Structure] = "The shell of the dwelling: walls and the openings in them, meaning doors, "
                              + "windows, cased openings. Walls that close off an area make a room by "
                              + "themselves; the Rooms tool says what each one is.",
        [HomeStage.Furnish]   = "Place furniture, fixtures and wall-mounted items like grab bars.",
        [HomeStage.SmartLiving] = "Everything that makes living here easier: sensing devices, everyday "
                              + "aids and the fixtures that go with them: what it costs, what it "
                              + "covers, and what a caregiver would be told.",
        [HomeStage.People]    = "Who lives here and what their day is. Their schedules and the clock "
                              + "place them in the plan.",
        [HomeStage.Review]    = "Compare a proposal to the base environment, and measure.",
        [HomeStage.Outdoors]  = "Entry ramps, walkways and railings around the home.",
    };

    public static string Tip(HomeStage stage) => Tips.TryGetValue(stage, out var t) ? t : null;
    ...
    /// UITheme.FitAll shortens on a narrow window. That is already handled: HomeEditController
    /// .StageTips leads a shortened tab's tooltip with the full stage name.
    /// </para>
    /// </remarks>
    public static string Label(HomeStage stage)
    {
    ...
    /// Measure.
    /// </summary>
    public static string PrimaryToolId(HomeStage stage) => ToolIdsFor(stage)[0];
    ...
    /// <para>
    /// It hides controls only. No stored data is touched: a home that already has
    /// <c>HomeDoc.exteriorEnabled</c> keeps the flag and keeps rendering its ramps and railings,
    /// <c>VariantDiff</c> keeps reporting outdoor changes, and the report keeps carrying them, so
    /// flipping this back leaves every home exactly where its author left it.
    /// </para>
    /// <para>
    ...
    public static List<HomeStage> VisibleStages(HomeDoc doc)
    {
        ...
        {
            HomeStage.Select, HomeStage.Import, HomeStage.Structure, HomeStage.Furnish, HomeStage.SmartLiving,
            HomeStage.People, HomeStage.Review,
        };
        if (OutdoorsUI && doc != null && doc.exteriorEnabled) stages.Add(HomeStage.Outdoors);
        return stages;
    }
    ...
    // No SELECT special case any more: "select" now resolves through the table to HomeStage.Select,
