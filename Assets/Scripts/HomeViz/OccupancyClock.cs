...
//
// A PLAIN CLASS, not a MonoBehaviour, owned by HomeRenderer and ticked from its Update. Two reasons it
...
//   * Not in the document. Every HomeDoc mutation is snapshotted for undo and marks the file dirty, so
//     a clock stored there would make each tick an undo entry and each playback an unsaved change.
//
// Being a plain class also means the whole occupants feature needs no scene edit: HomeRenderer is
// already wired in HomeViz.unity.
